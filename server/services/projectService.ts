...
export async function getMembers(projectId: string, userId: string) {
  await getProjectById(projectId, userId); // Verify access
...
export async function removeMember(projectId: string, userId: string, memberUserId: string) {
  const project = await getProjectById(projectId, userId);
